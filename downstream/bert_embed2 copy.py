...
class BERT(nn.Module):
    def __init__(self, vocab_size, id2embed):
        ...
        self.linear_prior = nn.Linear(id2embed.shape[1], vocab_size,False)
        self.linear_next = nn.Linear(id2embed.shape[1], vocab_size,False)
        self.linear_pred = nn.Linear(vocab_size, vocab_size)
    ...
    def forward(self, input_ids, masked_pos, user_ids=None, temp_ids=None):
        ...
        print(masked_pos_poi.shape)
        print(poi_embedding_tensor.shape)
        ...
        logits_lm = self.activ(self.linear_pred(logits_lm)+self.linear_prior(poi_prior)+ self.linear_next(poi_next))
        return logits_lm

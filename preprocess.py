...
if __name__ == '__main__':
    ...
    # h5_path = "data/h5_data/"
    # file_name = "000000_0"
    # read_row_data(row_path)
    df = pd.read_hdf(os.path.join('data/h5_data', "alldata_1" + ".h5"), key='data')
    df1 = pd.read_hdf(os.path.join('data/h5_data', "alldata_1" + ".h5"), key='data')
    df2 = pd.read_hdf(os.path.join('data/h5_data', "alldata_2" + ".h5"), key='data')
    raw_df = pd.concat([df1, df2], axis=0)
    dataset = DataSet(raw_df)
    seq_set = dataset.gen_seqence()
    pass
